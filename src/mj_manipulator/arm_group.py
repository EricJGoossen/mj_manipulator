...
def _read_site_pose(
    ...
    for i, site_id in enumerate(ee_site_id_group):
        tcp_offset = tcp_offset_group[i] if tcp_offset_group is not None else None
...
class ArmGroup(Mapping):
    ...
    def tcp_offset_group(self) -> list[np.ndarray] | None:
        ...
        return [arm.tcp_offset for arm in self.arms.values()]
    ...
    def get_joint_limits(self) -> list[tuple[np.ndarray, np.ndarray]]:
